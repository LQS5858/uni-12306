...
class Request {
  ...
  async get (url, config = {}, _config) {
    return this._request('get', url, config, _config);
  }
  async post (url, config = {}, _config) {
    return this._request('post', url, config, _config);
  }
  ...
  _request (method, url, config, _config) {
    const _this = this;
    ...
    return new Promise((resolve, reject) => {
      console.log('--hader--', lastConfig.header);
      uni.request({
        url: fullUrl,
        method,
